...
species_name_list = [i["comName"] for i in species_dict]
species_name_list.insert(0, "")
...
selected_species_multi = st.sidebar.multiselect(
    "Choose species to focus on", options=species_name_list[1:]
)
...
with maps:
    ...
    species_selected = st.selectbox("Select the species of bird", options=species_name_list)
...
# Line Plot Tab
with line_plot:
    species_selected_for_plot = st.selectbox("Select species for line plot", options=species_name_list)
...
# Bar Graph Tab
with bar_graph:
    species_selected_for_bar = st.selectbox("Select species for bar graph", options=species_name_list)
...
with table:
    ...
    if selection_choice == "Specific Data":
        if not df.empty:
            ...
            if species_filter:
                if simplify_data:
                    filtered_df = filtered_df.rename(columns={"comName": col[1],
                                                              "sciName": col[2],
                                                              "locName": col[4],
                                                              "howMany": col[6],
                                                              "lat": col[7],
                    ...
                    st.dataframe(filtered_df[[col[1], col[2],
                                              col[4], col[6],
                                              col[7], col[8]
                                              ]])
                else:
                    filtered_df = filtered_df.rename(columns={"speciesCode": col[0],
                                                              "comName": col[1],
                                                              "sciName": col[2],
                                                              "locId": col[3],
                                                              "locName": col[4],
                    ...
                    st.dataframe(filtered_df[[col[0], col[1], col[2], col[3], col[4], col[5],
        ...
        st.info("All information on recently observed bird species are shown below.")
        st.dataframe(df)
